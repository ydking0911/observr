...
<p align="center">
  <strong>AI 코딩 에이전트가 앱에서 무슨 일이 있었는지 볼 수 있습니다.</strong>
  <br/>
  <sub>로컬 개발 루프를 위한 한 줄 앱 계측 — AI 코딩 에이전트가 쿼리할 수 있는 구조화된 텔레메트리</sub>
</p>
...
observr는 하나의 교차점에 집중합니다: **앱 수준 트레이싱 · 로컬 개발 루프 · 한 줄 설정 · AI 코딩 에이전트가 쿼리할 수 있는 출력**. 각 도구는 자신의 영역에서 훌륭합니다. 차이점은 다음과 같습니다:

| | 앱 트레이싱 & 로그 | 계측 방법 | 로컬 / 인프라 불필요 | AI 코딩 에이전트가 쿼리 |
|--|:--:|:--:|:--:|:--:|
| **직접 로깅** | ✓ 수동 | 직접 연결 | ✓ | 직접 구현 |
| **Datadog** | ✓ | SDK + agent 설정 | ✗ 클라우드 | 유료 MCP 서버 경유 |
| **Grafana + OTel** | ✓ | OTel SDK 설정 | 셀프호스트 필요 | 외부 도구 경유 |
| **Netdata** | 인프라 / 커널 | 제로 코드 | ✓ | MCP (인프라 메트릭) |
| **Langfuse / Phoenix** | LLM 동작 | SDK | ✓ / 클라우드 옵션 | LLM 사이드 트레이스 |
| **observr** | **✓ 앱 요청, span, 로그** | **`init()` 1줄** | **✓ SQLite, 제로 인프라** | **CLI를 통한 구조화 JSON** |

> observr는 위 도구들을 대체하지 않습니다 — 각각 다른 문제를 잘 해결합니다. 경량 로컬 개발과 AI 소비 가능한 구조화 출력의 교차점을 채웁니다.

## observr가 하는 것 — 그리고 하지 않는 것

**하는 것:** 앱의 HTTP 요청, span, 로그를 한 줄 SDK 계측으로 로컬 SQLite에 저장하고, AI 코딩 에이전트(Claude Code, Cursor)가 구조화 JSON으로 쿼리할 수 있게 합니다.

**하지 않는 것:**
- LLM / AI 에이전트의 *동작* 관찰 — 그건 [Langfuse](https://langfuse.com/), Phoenix, Helicone 등이 잘 합니다 (다른 방향의 훌륭한 도구들)
- 인프라 또는 커널 수준 모니터링 — 그건 Netdata, Prometheus 등
- 클라우드 네이티브 운영 플랫폼 — 그건 Datadog, Grafana Cloud 등
...
| **v0.8** | 📋 | 네이티브 MCP 서버 — AI 코딩 에이전트(Claude Code, Cursor)가 CLI 없이 observr를 직접 쿼리 *(계획됨, 미구현)* |
