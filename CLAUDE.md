...
**observr** is a zero-config, open-source local observability stack for AI agents and developers. It consists of:
...
| Dashboard | React + Vite | `dashboard/` | Real-time event browser, embedded in `observrd` binary |
...
- **Broadcaster**: `storage.Store` holds a `Broadcaster` interface. In daemon mode, a `multiBroadcaster` fans out to both the WebSocket hub (`dashboard.Hub`) and the SSE tail hub (`tail.Hub`).
